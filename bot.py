...
from database import (
    init_db, init_cache_table, cache_get, cache_set, cache_cleanup,
    ensure_user,
...
ADMIN_ID         = 5555668323
CHANNEL_USERNAME = "@BetMindEdge"
WEBHOOK_URL      = "https://betbot-production-1cb7.up.railway.app"

# Текущая проходимость бота — меняй вручную в Railway Variables (BOT_WINRATE),
# раз в день, на основе своей статистики в Excel. Бот сам ничего не считает.
# Пусто/не задано — строка с винрейтом просто нигде не показывается.
BOT_WINRATE = os.environ.get("BOT_WINRATE", "").strip()
...
PLANS = {
    ...
    "pro_month":   {"name": "🔥 Pro",    "price": 799,  "days": 30,  "req_per_day": 10,   "web_search": True},
    "pro_year":    {"name": "🔥 Pro",    "price": 5990, "days": 365, "req_per_day": 10,   "web_search": True},
    # Скрытые тарифы — только через /activate
    "owner":   {"name": "👑 Owner",   "price": 0, "days": 36500, "req_per_day": 99999, "web_search": True},
    "owner_1": {"name": "👑 Owner №2","price": 0, "days": 36500, "req_per_day": 99999, "web_search": False},
}
...
FREE_LIMIT = 5  # запросов всего (не в день) для Free — только футбол
...
async def get_cs2_stats(team1: str, team2: str) -> str:
    """CS2 статистика через PandaScore: состав (/teams/{id}) + последние матчи"""
    try:
        async with httpx.AsyncClient(timeout=8) as client:
            ...
            async def get_full_team(team_id):
                """Отдельный запрос /teams/{id} — всегда возвращает полный состав"""
                r = await client.get(
                    f"https://api.pandascore.co/csgo/teams/{team_id}",
                    headers=headers
                )
            ...
            # Шаг 1: ищем ID обеих команд параллельно
            t1_short, t2_short = await asyncio.gather(search_team(team1), search_team(team2))

            if not t1_short or not t2_short:
                return ""

            t1_id, t2_id = t1_short["id"], t2_short["id"]

            # Шаг 2: параллельно берём полный состав и последние матчи (4 запроса)
            t1_full, t2_full, last1, last2 = await asyncio.gather(
                get_full_team(t1_id),
                get_full_team(t2_id),
                get_recent_matches(t1_id),
                get_recent_matches(t2_id),
            )

            def extract_players(team_data):
                players = team_data.get("players", [])
                return [p.get("name") or p.get("slug", "?") for p in players[:5]] if players else []

            p1 = extract_players(t1_full)
            p2 = extract_players(t2_full)

            stats = "📊 CS2 статистика (PandaScore):\n"
            stats += f"Состав {team1}: {', '.join(p1) if p1 else 'нет данных'}\n"
            stats += f"Состав {team2}: {', '.join(p2) if p2 else 'нет данных'}\n"
            if last1:
                stats += f"{team1} — последние матчи: {' | '.join(last1)}\n"
            if last2:
                stats += f"{team2} — последние матчи: {' | '.join(last2)}\n"
            stats += "(Составы из API — используй точно, не додумывай)\n"
            return stats
...
def detect_sport(text):
    ...
    hockey_words  = ["нхл","nhl","кхл","khl","хоккей","hockey",
                     "ovechkin","овечкин","crosby","maple leafs","rangers",
                     "bruins","penguins","lightning","avalanche","capitals",
                     "cska hockey","ska ","динамо хоккей","локомотив хоккей"]
    bball_words   = ["нба","nba","баскетбол","basketball","lakers","celtics",
                     "warriors","bulls","heat","nets","bucks","suns","nuggets",
                     "clippers","knicks","76ers","raptors","euroleague","евролига"]
    for w in cs2_words:
        if w in t: return "cs2"
    for w in dota_words:
        if w in t: return "dota"
    for w in hockey_words:
        if w in t: return "hockey"
    for w in bball_words:
        if w in t: return "basketball"
    return "football"
...
async def get_hockey_stats(team1: str, team2: str) -> str:
    """Хоккейная статистика через PandaScore"""
    try:
        async with httpx.AsyncClient(timeout=8) as client:
            headers = {"Authorization": f"Bearer {PANDASCORE_KEY}"}

            async def search_team(name):
                r = await client.get("https://api.pandascore.co/hockey/teams",
                    params={"search[name]": name, "per_page": 1}, headers=headers)
                data = r.json()
                return data[0] if data and isinstance(data, list) else None

            async def get_recent(team_id):
                r = await client.get("https://api.pandascore.co/hockey/matches/past",
                    params={"filter[opponent_id]": team_id, "per_page": 3, "sort": "-begin_at"},
                    headers=headers)
                matches = r.json()
                results = []
                if isinstance(matches, list):
                    for m in matches:
                        ...
                        teams = " vs ".join([o.get("opponent", {}).get("name", "?") for o in opponents])
                        results.append(f"{teams} → {winner}")
                return results

            t1, t2 = await asyncio.gather(search_team(team1), search_team(team2))
            if not t1 or not t2:
                return ""
            last1, last2 = await asyncio.gather(get_recent(t1["id"]), get_recent(t2["id"]))
            stats = "📊 Хоккей статистика (PandaScore):\n"
            if last1:
                stats += f"{team1} — последние матчи: {' | '.join(last1)}\n"
            if last2:
                stats += f"{team2} — последние матчи: {' | '.join(last2)}\n"
        ...
        logger.warning(f"PandaScore hockey error: {e}")
        return ""


async def get_basketball_stats(team1: str, team2: str) -> str:
    """Баскетбольная статистика через PandaScore"""
    try:
        async with httpx.AsyncClient(timeout=8) as client:
            headers = {"Authorization": f"Bearer {PANDASCORE_KEY}"}

            async def search_team(name):
                r = await client.get("https://api.pandascore.co/basketball/teams",
                    params={"search[name]": name, "per_page": 1}, headers=headers)
                data = r.json()
                return data[0] if data and isinstance(data, list) else None

            async def get_recent(team_id):
                r = await client.get("https://api.pandascore.co/basketball/matches/past",
                    params={"filter[opponent_id]": team_id, "per_page": 3, "sort": "-begin_at"},
                    headers=headers)
                matches = r.json()
                results = []
            ...
            if not t1 or not t2:
                return ""
            last1, last2 = await asyncio.gather(get_recent(t1["id"]), get_recent(t2["id"]))
            stats = "📊 Баскетбол статистика (PandaScore):\n"
            if last1:
                stats += f"{team1} — последние матчи: {' | '.join(last1)}\n"
            if last2:
                stats += f"{team2} — последние матчи: {' | '.join(last2)}\n"
        ...
        logger.warning(f"PandaScore basketball error: {e}")
        return ""
...
def _team_matches_one(query: str, name: str) -> bool:
    """Проверяем совпадение одной команды с одним названием из API (без 'пустая строка матчит всё')"""
    if not name:
        return False
    q = _normalize_team(query)
    n = name.lower()
    return q in n or n[:6] in q

def _teams_match(query: str, home: str, away: str) -> bool:
    """Проверяем совпадение команды с home/away из API"""
    return _team_matches_one(query, home) or _team_matches_one(query, away)

def _implied_prob(decimal_odds: float) -> float:
    """Подразумеваемая вероятность из десятичного коэффициента (без вычета маржи букмекера)"""
    try:
        return round(100 / float(decimal_odds), 1)
    except (ValueError, ZeroDivisionError):
        return 0.0

async def get_odds(team1: str, team2: str, sport: str) -> str:
    """Получаем актуальные коэффициенты букмекеров по нескольким рынкам:
    h2h (исход), totals (тотал), spreads (фора).
    Считаем implied probability по каждому исходу как справочный ориентир рынка —
    Claude использует коэффициенты для расчёта итоговых ставок по самым вероятным исходам.
    btts (обе забьют) сознательно НЕ запрашиваем — это не featured market для многих
    лиг, и при включении в общий запрос ронял весь запрос с 422 (Odds API отклоняет
    всю пачку markets при невалидном/неподдерживаемом среди них). Если нужна оценка
    BTTS — Claude может прикинуть её сам по статистике без точного коэффициента."""
    if sport not in ("football", "cs2"):
        # Odds API покрывает только футбол (топ-лиги) и CS2 — для хоккея/баскетбола/dota нет смысла стучаться
        return ""

    async def fetch_league(client: httpx.AsyncClient, sport_key: str, params: dict):
        try:
            r = await client.get(
            ...
            if r.status_code != 200:
                # Логируем тело ответа — Odds API обычно явно пишет причину
                # (невалидный рынок, ключ, регион и т.д.), это сильно ускоряет диагностику
                logger.warning(f"Odds API {sport_key}: HTTP {r.status_code} | {r.text[:300]}")
                return []
            events = r.json()
            return events if isinstance(events, list) else []
        except Exception as e:
            logger.warning(f"Odds API league fetch error ({sport_key}): {e}")
            return []

    # Рынки: h2h + totals + spreads — featured markets, всегда доступны через основной
    # /odds эндпоинт для обоих видов спорта.
    ...
    try:
        async with httpx.AsyncClient(timeout=8) as client:
            ...
            sport_keys = ["esports_cs2"] if sport == "cs2" else FOOTBALL_SPORT_KEYS

            # Бьём по всем лигам параллельно вместо последовательного перебора —
            # быстрее в разы и не упирается в общий таймаут на каждую лигу по очереди
            results = await asyncio.gather(
                *(fetch_league(client, key, params) for key in sport_keys)
            )
            ...
            for events in results:
                for event in events:
                    ...
                    # Матч засчитываем только если ОБЕ команды совпали — одна с home, другая с away
                    # (иначе можно подсунуть коэффициенты другого матча с одной из тех же команд)
                    match_ok = (
                        (_team_matches_one(team1, home) and _team_matches_one(team2, away))
                        or (_team_matches_one(team2, home) and _team_matches_one(team1, away))
                    )
                    if not match_ok:
                        continue
                    ...
                    # Собираем коэффициенты по рынкам отдельно, чтобы не дублировать на каждого букмекера
                    market_labels = {
                        "h2h": "🏆 Исход (1X2)",
                        "totals": "⚖️ Тотал",
                        "spreads": "📐 Фора",
                    }
                    collected = {k: [] for k in market_labels}
                    ...
                    for bm in bookmakers:
                        ...
                        for market in bm.get("markets", []):
                            ...
                            for o in outcomes:
                                ...
                                price = o.get("price")
                                point = o.get("point")
                                prob = _implied_prob(price) if price else 0.0
                                label = f"{name} {point}" if point is not None else name
                                pieces.append(f"{label}: {price} (~{prob}%)")
                            if pieces:
                                collected[mkey].append(f"{bm_name}: " + " | ".join(pieces))

                    odds_info = f"\n💰 Коэффициенты букмекеров ({home} vs {away}):\n"
                    has_any = False
                    for mkey, label in market_labels.items():
                        lines = collected.get(mkey, [])
                        if lines:
                            has_any = True
                            odds_info += f"{label}:\n  " + "\n  ".join(lines) + "\n"
                    ...
                    odds_info += (
                        "(~% — implied probability букмекера, без вычета маржи; "
                        "справочный ориентир рынка. Используй коэффициенты для расчёта "
                        "находок среди исходов с твоей оценкой вероятности ≥ 60%)\n"
                )
                return odds_info
...
# ══════════════════════════════════════════════════════════════════════════════
SYSTEM_PROMPT = """Ты — спортивный аналитик и автор популярного betting-канала. Дата: {current_date}.
Твоя аудитория читает с телефона за 10-15 секунд — пиши так, чтобы зацепило сразу,
а не как отчёт. Ты эксперт с мнением, а не таблица вероятностей.

Твоя задача — посмотреть на матч в целом и предложить ОДНУ главную ставку
(и, только если она реально того стоит, одну запасную) из ВСЕГО пула рынков:
исход (П1/Х/П2), двойной шанс, тотал, фора, обе забьют, результат/тотал
первого тайма (если есть статистика по тому, как команды играют 1-й тайм).

Как работать:
1. Проанализируй форму, статистику, личные встречи, мотивацию, состав команд.
2. Найди САМЫЙ яркий, конкретный факт о матче — то, что цепляет внимание
   (контраст формы, явный перекос статистики, важный игрок, психологический
   подтекст). Это будет крючок в начале поста.
3. Выбери ОДНУ главную ставку — ту, что ты сам поставил бы не задумываясь.
   Не подбирай вариант под коэффициент, который и так очевиден рынку
   (например, фаворит по коэффициенту ~1.05-1.15 — это не ставка, это
   констатация факта; такое в "Ставку дня" не выноси).
4. Запасной вариант добавляй ТОЛЬКО если у него есть отдельное, не менее
   сильное обоснование, чем у главной ставки. Если такого нет — не пиши
   второй вариант вообще, одна сильная ставка лучше двух слабых.
5. Коэффициенты бери ТОЛЬКО из блока данных или со скриншота, если они есть.
   Если коэффициента по рынку нет — можно предложить вариант без кэфа, опираясь
   на свою экспертную оценку, но не выдумывай цифры коэффициентов.
6. КРИТИЧЕСКИ ВАЖНО — выводи ТОЛЬКО финальный результат, без хода рассуждений.
   Не показывай пользователю перебор отброшенных вариантов и не добавляй
   примечания о нехватке данных. Весь отсев делай молча про себя, в ответе — только итог.

Формат ответа — строго по структуре ниже, без лишних слов:

🔥 *Команда1 — Команда2* | Турнир

[ОДНА хлёсткая строка-крючок — самый яркий факт или конфликт сюжета матча.
Без вступления типа "матч обещает быть интересным" — сразу суть.]

📌 *Что нужно знать:*
• [самый сильный аргумент в пользу ставки]
• [второй факт, если усиливает картину]
• [третий — только если реально по делу, не для галочки]

🎯 *Ставка дня:* [рынок и название] | К X.XX | уверенность X%
→ [1 строка живым языком — почему именно это, без канцелярита]

[Если есть весомый запасной вариант — этот блок, иначе пропусти целиком:]
🎲 *Запасной вариант:* [рынок и название] | К X.XX | уверенность X%
→ [1 строка — почему]

Правила:
- Главная ставка — практически всегда одна. Два варианта — редкое исключение,
  а не стандарт. Если сильного второго нет — НЕ добавляй блок "Запасной вариант".
- Никогда не выноси в ставку явного фаворита по "убитому" коэффициенту
  (примерно ниже 1.20) без отдельного веского аргумента — это не помогает
  читателю и выглядит как заполнение шаблона.
- Рынки для поиска: исход (П1/Х/П2), двойной шанс (1X/X2), тотал (ТБ/ТМ X.5),
  фора (Ф1/Ф2 ±X.5), обе забьют (Да/Нет), результат/тотал 1-го тайма.
  Угловые, удары в створ и другую экзотику не анализируй — для них нет
  надёжных данных, не придумывай цифры.
- Для хоккея/баскетбола (где нет блока коэффициентов с API): анализируй
  вратарей/лидеров по очкам, форму, статистику — давай свою экспертную оценку.
- Уверенность ставки от 55% до 85%. Высокую уверенность (75%+) ставь только
  когда аргументы реально это оправдывают — она не должна быть в каждом посте,
  иначе обесценивается.
- Состав команд указывай ТОЛЬКО если данные есть в блоке API/скриншоте, и только
  если это нужно для крючка или аргумента — не вставляй списком ради списка.
- Пиши живым голосом эксперта, не отчётом: короткие фразы, без канцелярита
  вроде "состояние: под давлением" — лучше "Испания обязана отыграться".
- Только факты и цифры, никакой воды. Никаких слов "вероятность" или "value" —
  только "уверенность %".
...
async def analyze_match(user_text: str, sport: str,
    ...
    sport_context = {
        ...
        "hockey":     "ХОККЕЙ. Анализируй форму команд, голкиперов, последние матчи, статистику голов, игру в большинстве/меньшинстве.",
        "basketball": "БАСКЕТБОЛ. Анализируй форму команд, лидеров по очкам, последние матчи, темп игры, травмы ключевых игроков.",
        "cs2":        "CS2. Состав берёшь ТОЛЬКО из блока 📊 CS2 статистика — не додумывай игроков. Анализируй рейтинг HLTV, карточный пул, форму команд, последние турнирные результаты.",
        "dota":       "DOTA 2. Анализируй винрейт, текущую мету, стиль игры, пик-фазу, последние результаты. Состав берёшь только из данных API если есть.",
    }
...
FAQ_TEXT = (
    ...
    "  • ESPN, FlashScore, SofaScore\n"
    "\n"
    "━━━━━━━━━━━━━━━━━━━━\n"
    "📊 Что показывает анализ?\n"
    "━━━━━━━━━━━━━━━━━━━━\n"
    "Бот даёт компактный экспертный разбор:\n"
    "  • Главный факт о матче — что реально важно знать\n"
    "  • Ставку дня с коэффициентом и уверенностью %\n"
    "  • Запасной вариант — если он того стоит\n"
    "  • Ключевые факторы в 1-2 строки\n"
    "\n"
    "━━━━━━━━━━━━━━━━━━━━\n"
    "⚽ Какие виды спорта поддерживаются?\n"
    "━━━━━━━━━━━━━━━━━━━━\n"
    "• Футбол — топ-лиги Европы, еврокубки, ЧМ\n"
    "• Хоккей — НХЛ, КХЛ\n"
    ...
    "\n"
    "━━━━━━━━━━━━━━━━━━━━\n"
    "🆓 Чем Free отличается от Basic и Pro?\n"
    "━━━━━━━━━━━━━━━━━━━━\n"
    "Free — 5 анализов (только футбол, без подписки)\n"
    "Basic — 5 анализов в день, все виды спорта\n"
    "Pro — 10 анализов в день + web-поиск актуальных составов и новостей\n"
    "\n"
    "━━━━━━━━━━━━━━━━━━━━\n"
...
async def cmd_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    winrate_stats_line = f"📈 Проходимость бота: {BOT_WINRATE}%\n\n" if BOT_WINRATE else ""

    text = (
        f"👤 *Моя статистика*\n\n"
        f"{winrate_stats_line}"
        f"{plan_info}"
...
async def cmd_guide(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    guide_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "betmind_guide.pdf")
    if not os.path.exists(guide_path):
        await update.message.reply_text("📚 Гайд временно недоступен. Попробуй позже.")
        return
    await context.bot.send_document(
        chat_id=user.id,
        document=open(guide_path, "rb"),
        filename="BetMind_Bankroll_Guide.pdf",
        caption=(
            "📚 *Гайд: Как не сливать банкролл*\n\n"
            "Внутри:\n"
            "• Управление банкроллом\n"
            "• Критерий Келли\n"
            "• Value-ставки\n"
            "• Психология и типичные ошибки\n\n"
            "Читай, внедряй — и присылай скриншот матча для анализа 📸"
        ),
        parse_mode=ParseMode.MARKDOWN
    )
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if plan:
        ...
        status = f"🎁 Бесплатных анализов: *{free_left}* из {FREE_LIMIT} (только ⚽ футбол)"

    winrate_line = f"\n📈 Проходимость ставок: *{BOT_WINRATE}%*\n" if BOT_WINRATE else "\n"

    text = (
        f"👋 Привет, *{user.first_name}*!\n\n"
        f"🧠 Я — *BetMind Bot*\n"
        f"ИИ-аналитик матчей по скриншоту — за 30 секунд получишь прогноз с реальной статистикой.\n"
        f"{winrate_line}"
        f"📸 *Как пользоваться:*\n"
        f"Пришли скриншот матча из букмекера, Flashscore, Sofascore, HLTV или Liquipedia —\n"
        f"бот сам распознает команды и выдаст анализ.\n\n"
        f"{status}\n\n"
        f"⚽🏒🏀🎮🐉 Футбол • Хоккей • Баскетбол • CS2 • Dota 2\n"
        f"_Free: только футбол. Все виды спорта — по подписке._"
    )
    keyboard = [
        [InlineKeyboardButton("📸 Отправить скриншот — получить анализ", callback_data="how_to_use")],
        [InlineKeyboardButton("📣 Канал с результатами", url="https://t.me/BetMindEdge")],
        [InlineKeyboardButton("💎 Тарифы и подписка", callback_data="show_plans")],
        [InlineKeyboardButton("📚 Гайд: как не сливать банкролл", callback_data="get_guide")],
...
async def show_plans(message):
    text = (
        ...
        "🔥 *Pro — 10 анализов в день*\n"
        "├ 799₽/месяц\n"
        "└ 5 990₽/год _(экономия 37%)_ 🏆\n"
        "└ + web-поиск свежих новостей\n\n"
        "🆓 *Free* — 5 футбольных анализов (всего, без подписки)\n\n"
...
async def cmd_activate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if plan_key not in PLANS:
            await update.message.reply_text("Неверный план. Используй: basic_month, basic_year, pro_month, pro_year, owner, owner_1")
            return
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "how_to_use":
        instruction = (
            "📸 *Как сделать скриншот для анализа:*\n\n"
            "На скриншоте *обязательно* должно быть видно:\n"
            "✅ Названия обеих команд\n"
            "✅ Вид спорта\n\n"
            "Желательно, но не обязательно:\n"
            "☑️ Название турнира\n"
            "☑️ Коэффициенты букмекера\n\n"
            "Подходят скриншоты из:\n"
            "• Любого букмекера (1xBet, Фонбет, Марафон...)\n"
            "• Flashscore, Sofascore\n"
            "• HLTV, Liquipedia\n\n"
            "👇 *Примеры правильных скриншотов:*"
        )
        await query.message.reply_text(instruction, parse_mode=ParseMode.MARKDOWN)
        import os
        for ex_path in ["example_screenshots/example1.png", "example_screenshots/example2.png"]:
            if os.path.exists(ex_path):
                with open(ex_path, "rb") as f:
                    await context.bot.send_photo(chat_id=uid, photo=f)
        ...
        guide_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "betmind_guide.pdf")
        if not os.path.exists(guide_path):
            await query.message.reply_text("📚 Гайд временно недоступен. Попробуй позже.")
        else:
            await context.bot.send_document(
                chat_id=uid,
                document=open(guide_path, "rb"),
...
def build_cache_key(analysis_text: str) -> str | None:
    """Строим ключ кеша из текста анализа: sport:team1:team2"""
    # Ищем только в первых 3 строках — там всегда заголовок "Команда1 vs Команда2 | Турнир"
    first_lines = "\n".join(analysis_text.strip().splitlines()[:3])
    match_line = _re.search(
        r'([А-ЯA-Z][\w\s\-\.]{1,30}?)\s+(?:vs|против)\s+([А-ЯA-Z][\w\s\-\.]{1,30})',
        first_lines, _re.IGNORECASE
    )
    # Определяем вид спорта по всему тексту
    sport_match = _re.search(
        r'(футбол|football|хоккей|hockey|баскетбол|basketball|cs2|dota)',
        analysis_text, _re.IGNORECASE
    )
    if not match_line:
        return None
    sport = sport_match.group(1).lower().strip() if sport_match else "sport"
    team1 = " ".join(match_line.group(1).lower().split()).strip(" *_")
    team2 = " ".join(match_line.group(2).lower().split()).strip(" *_")
    teams = sorted([team1, team2])
    return f"{sport}:{teams[0]}:{teams[1]}"


ODDS_REFRESH_SECONDS = 24 * 3600  # раз в сутки пересобираем анализ со свежими коэффициентами

def _pack_cache_value(analysis: str) -> str:
    """Заворачиваем текст анализа вместе с меткой времени для проверки свежести коэффициентов"""
    return json.dumps({"ts": datetime.utcnow().isoformat(), "text": analysis}, ensure_ascii=False)

def _unpack_cache_value(raw: str) -> tuple[str | None, bool]:
    """
    Достаём текст анализа и флаг 'свежий ли ещё' (младше ODDS_REFRESH_SECONDS).
    Возвращает (текст_или_None, is_fresh). Если формат старый/битый — считаем устаревшим,
    чтобы он сам пересобрался на ближайшем запросе.
    """
    try:
        data = json.loads(raw)
        ts = datetime.fromisoformat(data["ts"])
        age = (datetime.utcnow() - ts).total_seconds()
        return data["text"], age < ODDS_REFRESH_SECONDS
    except Exception:
        return raw, False


async def run_cache_cleanup():
    """Ежедневная чистка устаревших записей кеша (7 дней без обращений)"""
    while True:
        await asyncio.sleep(86400)
        try:
            deleted = await cache_cleanup(days=7)
            logger.info(f"Cache cleanup: removed {deleted} old entries")
        except Exception as e:
            logger.warning(f"Cache cleanup error: {e}")


async def extract_match_from_screenshot(image_bytes: bytes) -> dict | None:
    """Быстрый лёгкий запрос — только вытащить команды и вид спорта из скрина"""
    import base64
        ...
        messages=[{
            ...
            "content": [
                ...
                },
                {
                    "type": "text",
                    "text": (
                        "На скриншоте спортивный матч. Ответь ТОЛЬКО в формате JSON, ничего лишнего:\n"
                        "{\"team1\": \"Название команды 1\", \"team2\": \"Название команды 2\", "
                        "\"sport\": \"football|hockey|basketball|cs2|dota\"}\n"
                        "Если не можешь определить — верни {\"error\": \"unknown\"}"
                    )
                }
            ]
        }]
...
async def analyze_screenshot(image_bytes: bytes, plan: str | None, odds: str = "") -> str:
    ...
    odds_block = f"\n\nРеальные коэффициенты букмекеров на этот матч (используй их вместо собственной оценки):\n{odds}" if odds else ""
    ...
    user_msg = (
        ...
        "начни с цепляющего факта, выбери ОДНУ главную ставку из всех рынков "
        "(включая 1-й тайм, если уместно), и добавь запасной вариант только если "
        "он реально того стоит. "
        "Без рассуждений вслух и без примечаний о недостающих на скриншоте данных. "
        "Максимум 10 строк."
        f"{odds_block}"
    )
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # ── Шаг 1: быстрое извлечение команд и спорта (~2-3 сек) ────────
        match_info = await extract_match_from_screenshot(image_bytes)
        logger.info(f"extract_match_from_screenshot: {match_info}")
        ...
        extracted_sport = match_info.get("sport", "")
        team1 = match_info.get("team1", "")
        team2 = match_info.get("team2", "")

        # ── Проверка вида спорта для Free ────────────────────────────────
        if not plan and extracted_sport != "football":
            await wait_msg.edit_text(
                "⛔ *Free — только футбол*\n\n"
                "Бесплатный тариф поддерживает анализ только футбольных матчей.\n\n"
                "Для хоккея, баскетбола, CS2 и Dota 2 — оформи подписку:",
                parse_mode=ParseMode.MARKDOWN,
        ...
        # ── Шаг 2: проверяем кеш по командам ────────────────────────────
        is_pro = plan and plan.startswith(("pro", "owner"))
        teams_sorted = sorted([team1.lower().strip(), team2.lower().strip()])
        cache_key = f"{extracted_sport}:{teams_sorted[0]}:{teams_sorted[1]}"
        cached_raw = await cache_get(cache_key)

        cached_text, is_fresh = (None, False)
        if cached_raw:
            cached_text, is_fresh = _unpack_cache_value(cached_raw)

        if cached_text and is_fresh:
            logger.info(f"Cache HIT (fresh, <24h): {cache_key}")
            final_analysis = cached_text
        else:
            # ── Шаг 3: полный анализ — кеша нет ИЛИ коэффициенты устарели (>24ч) ──
            reason = "stale (>24h)" if cached_text else "miss"
            logger.info(f"Cache {reason} — running full analysis: {cache_key}")

            odds = ""
            try:
                odds = await get_odds(team1, team2, extracted_sport)
                if odds:
                    logger.info(f"Odds found for {cache_key}")
            except Exception as e:
                logger.warning(f"get_odds failed for {cache_key}: {e}")

            analysis = await analyze_screenshot(image_bytes, plan, odds=odds)

            if not analysis:
                if cached_text:
                    # Не смогли пересобрать — лучше отдать чуть устаревший анализ, чем ничего
                    logger.warning(f"Refresh failed, falling back to stale cache: {cache_key}")
                    final_analysis = cached_text
                else:
                    await wait_msg.edit_text("❌ Не удалось проанализировать матч. Попробуй прислать другой скриншот.")
                    return
            else:
                final_analysis = analysis
                if is_pro:
                    await cache_set(cache_key, _pack_cache_value(analysis))
                    logger.info(f"Cache SET (fresh): {cache_key}")
        ...
        if plan:
            ...
            free_left = max(0, FREE_LIMIT - free_used)
            if free_left == 0:
                counter_info = "🔴 Это был последний бесплатный анализ."
            elif free_left == 1:
                counter_info = f"⚠️ Остался *1* бесплатный анализ — последний!"
            else:
                counter_info = f"🎁 Осталось бесплатных: {free_left}"

        sport_emoji = "📸"
        winrate_footer_line = f"📈 Проходимость: {BOT_WINRATE}%\n" if BOT_WINRATE else ""
        footer = (
            f"\n\n{sport_emoji} SCREENSHOT • 📅 {datetime.now().strftime('%d.%m.%Y %H:%M')}\n"
            f"{winrate_footer_line}"
            f"{counter_info}\n"
        ...
        full_text = final_analysis + footer
        ...
        # Upsell после последнего / предпоследнего бесплатного анализа
        if not plan:
            free_used_now = await get_free_used(uid)
            free_left_now = max(0, FREE_LIMIT - free_used_now)
            if free_left_now == 0:
                ...
                await update.message.reply_text(
                    ...
                    "Вчера бот дал точные прогнозы на все матчи ЧМ-2026 📊\n\n"
                    "Оформи подписку и продолжай получать анализы:\n"
                    "🔵 Basic — 5 анализов в день, 399₽/мес\n"
                    "🔥 Pro — 10 анализов + web-поиск, 799₽/мес",
                    parse_mode=ParseMode.MARKDOWN,
                    reply_markup=upsell_kb
                )
            elif free_left_now == 1:
                await update.message.reply_text(
                    "⚠️ *Остался 1 бесплатный анализ*\n\n"
                    "После него бот будет недоступен без подписки.\n"
                    "Оформи сейчас — Basic от 399₽/мес 👇",
                    parse_mode=ParseMode.MARKDOWN,
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("💎 Выбрать тариф", callback_data="show_plans")]
                    ])
