...
STRATZ_KEY       = os.environ["STRATZ_KEY"]
...
async def get_cs2_stats(team1: str, team2: str) -> str:
    """Получаем CS2 статистику через PandaScore"""
    try:
        async with httpx.AsyncClient(timeout=8) as client:
            ...
            async def get_recent_matches(team_id, team_name):
                r = await client.get(
                    f"https://api.pandascore.co/csgo/matches/past",
                    params={"filter[opponent_id]": team_id, "per_page": 5, "sort": "-begin_at"},
                    headers=headers
                )
                matches = r.json()
                results = []
            ...
            last1 = await get_recent_matches(t1["id"], team1)
            last2 = await get_recent_matches(t2["id"], team2)

            stats = f"📊 CS2 статистика (PandaScore):\n"
            if last1:
                stats += f"{team1} — последние матчи: {' | '.join(last1[:3])}\n"
            if last2:
                stats += f"{team2} — последние матчи: {' | '.join(last2[:3])}\n"

            # Рейтинг команд
            # Получаем предстоящие матчи и коэффициенты
            r_upcoming = await client.get(
                "https://api.pandascore.co/csgo/matches/upcoming",
                params={"per_page": 50},
                headers=headers
            )
            upcoming = r_upcoming.json()
            if isinstance(upcoming, list):
                for m in upcoming:
                    opponents = [o.get("opponent", {}).get("name", "").lower() for o in m.get("opponents", [])]
                    if team1.lower()[:4] in " ".join(opponents) or team2.lower()[:4] in " ".join(opponents):
                        match_time = m.get("begin_at", "")[:10] if m.get("begin_at") else "скоро"
                        stats += f"Предстоящий матч: {match_time}\n"
                        break
            stats += f"Рейтинг {team1}: #{t1.get('id', '?')} | Рейтинг {team2}: #{t2.get('id', '?')}\n"
            return stats
...
async def get_dota_stats(team1: str, team2: str) -> str:
    ...
    try:
        async with httpx.AsyncClient(timeout=8) as client:
            ...
            # Ищем команды через Stratz
            query = """
            {
                teams(request: {take: 3, teamName: "%s"}) {
                    id
                    name
                    winCount
                    lossCount
            ...
            data = r.json()
            teams1 = data.get("data", {}).get("teams", [])

            query2 = query.replace(team1, team2)
            ...
            data2 = r2.json()
            teams2 = data2.get("data", {}).get("teams", [])
            ...
            def winrate(t):
                w = t.get("winCount", 0) or 0
                l = t.get("lossCount", 0) or 0
                total = w + l
                return f"{round(w/total*100)}%" if total > 0 else "N/A"

            stats = (
                f"📊 Dota 2 статистика (Stratz):\n"
                f"{team1}: побед {t1.get('winCount', 0)}, поражений {t1.get('lossCount', 0)}, "
                f"винрейт {winrate(t1)}\n"
                f"{team2}: побед {t2.get('winCount', 0)}, поражений {t2.get('lossCount', 0)}, "
                f"винрейт {winrate(t2)}\n"
            )
            return stats

    except Exception as e:
        logger.warning(f"Stratz Dota error: {e}")
        return ""
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Коэффициенты только для футбола (Odds API не поддерживает CS2/Dota)
    odds_coro = get_odds(team1, team2, sport) if (team2 and sport == "football") else empty()
