...
async def analyze_match(user_text: str, sport: str,
    ...
    sport_context = {
        ...
        "cs2":        "CS2. Анализируй по доступным данным.",
        "dota":       "DOTA 2. Анализируй по доступным данным.",
    }
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        quick_sport = detect_sport(caption) if caption else None
        if quick_sport in ("cs2", "dota"):
            await wait_msg.edit_text(
                "🎮 *Киберспорт — раздел в разработке*\n\n"
                "Мы работаем над качественной аналитикой CS2 и Dota 2.\n"
                "Пока раздел временно недоступен — чтобы не давать некачественные прогнозы.\n\n"
                "⚽🏒🏀 Бот работает для *футбола, хоккея и баскетбола* — попробуй!",
                parse_mode=ParseMode.MARKDOWN
            )
            return
        ...
        if analysis:
            detected = detect_sport(analysis)
            if detected in ("cs2", "dota"):
                await wait_msg.edit_text(
                    "🎮 *Киберспорт — раздел в разработке*\n\n"
                    "Мы работаем над качественной аналитикой CS2 и Dota 2.\n"
                    "Пока раздел временно недоступен — чтобы не давать некачественные прогнозы.\n\n"
