...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if cached:
            logger.info(f"Cache HIT (fast): {cache_key}")
            final_analysis = cached
        else:
            # ── Шаг 3: полный анализ только при промахе кеша ─────────────
            logger.info(f"Cache MISS — running full analysis: {cache_key}")
            analysis = await analyze_screenshot(image_bytes, plan)
