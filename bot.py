...
def build_cache_key(analysis_text: str) -> str | None:
    ...
    # Ищем строку вида "Команда1 vs/против/— Команда2" без привязки к слову "футбол"
    match_line = _re.search(
        r'([А-ЯA-Z\w][а-яёa-z\w\s\-\.]{1,30}?)\s+(?:vs|против|\u2014)\s+([А-ЯA-Z\w][а-яёa-z\w\s\-\.]{1,30})',
        analysis_text, _re.IGNORECASE
    )
    # Определяем вид спорта отдельно по всему тексту
    ...
    team1 = " ".join(match_line.group(1).lower().split())
    team2 = " ".join(match_line.group(2).lower().split())
