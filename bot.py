...
💎 *Три лучшие находки (по убыванию value):*
...
Правила:
- Три находки — НЕ обязательно разных типов. Бери ЛУЧШИЕ по расхождению вероятностей,
  даже если все три из одного рынка (например, три разных тотала) — если это даёт
  максимальный value, так и делай. Приоритет — реальное расхождение, а не разнообразие.
    ...
    if os.path.exists(guide_path):
        with open(guide_path, "rb") as f:
            await update.message.reply_document(
                ...
                caption=(
                    "📚 *Руководство по управлению банкроллом*\n\n"
                    "_Как не сливать банкролл и ставить с умом_ 🎯\n\n"
                    "Читай перед ставками — сохраняет деньги лучше любого прогноза."
                ),
                parse_mode=ParseMode.MARKDOWN
            )
    else:
        await update.message.reply_text(
            "⚠️ Руководство временно недоступно. Попробуй позже."
        )
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("📚 Гайд: как не сливать банкролл", callback_data="send_guide")],
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "how_to_use":
        ...
        if os.path.exists(guide_path):
            ...
            await query.message.reply_text("⚠️ Руководство временно недоступно. Попробуй позже.")
...
async def analyze_screenshot(image_bytes: bytes, plan: str | None, odds: str = "") -> str:
    ...
    user_msg = (
        ...
        "найди три лучшие value-находки (расхождение твоей оценки и коэффициента) "
