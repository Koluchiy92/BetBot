...
# ══════════════════════════════════════════════════════════════════════════════
#  PANDASCORE — Dota 2 статистика
# ══════════════════════════════════════════════════════════════════════════════
async def get_dota_stats(team1: str, team2: str) -> str:
    """Получаем Dota 2 статистику через PandaScore"""
    try:
        async with httpx.AsyncClient(timeout=8) as client:
            headers = {"Authorization": f"Bearer {PANDASCORE_KEY}"}

            # Алиасы — сокращения к полным названиям команд в PandaScore
            DOTA_ALIASES = {
                "bb team": "BetBoom",
                "bb": "BetBoom",
                "betboom": "BetBoom",
            ...
            async def search_team(name):
                ...
                search_name = DOTA_ALIASES.get(name.lower().strip(), name)
                r = await client.get(
                    "https://api.pandascore.co/dota2/teams",
                    params={"search[name]": search_name, "per_page": 1},
                    headers=headers
                )
                data = r.json()
                if data and isinstance(data, list):
                    logger.info(f"PandaScore Dota2 found '{name}' → '{data[0].get('name')}' (id={data[0].get('id')})")
                    return data[0]
                logger.warning(f"PandaScore Dota2: команда '{name}' (искал '{search_name}') не найдена")
                return None
            ...
            async def get_recent_matches(team_id):
                ...
                matches = r.json()
                # Временный дебаг — смотрим структуру первого матча
                if matches and isinstance(matches, list):
                    m0 = matches[0]
                    logger.info(f"DEBUG match fields: winner={m0.get('winner')}, "
                                f"results={m0.get('results')}, "
                                f"games_winners={[g.get('winner') for g in (m0.get('games') or [])]}")
                results = []
                if isinstance(matches, list):
                    for m in matches:
                        opponents = m.get("opponents", [])
                        ...
                        # Победитель — пробуем несколько полей
                        winner = m.get("winner") or {}
                        w_name = winner.get("name") if winner else None
                        # Если winner пустой — смотрим results
                        if not w_name:
                            for game in (m.get("results") or []):
                                if game.get("winner"):
                                    w_name = game.get("winner", {}).get("name")
                                    break
                        w_name = w_name or "?"
                        results.append(f"{teams} → {w_name} ({tournament})")
                return results
            ...
            if t1["id"] == t2["id"]:
                logger.warning(f"PandaScore Dota2: обе команды один ID {t1['id']} — пропускаем")
                return ""

            # Параллельный запрос матчей
            last1, last2 = await asyncio.gather(
                get_recent_matches(t1["id"]),
                get_recent_matches(t2["id"])
            )

            stats = "📊 Dota 2 статистика (PandaScore):\n"
            if last1:
        ...
        logger.warning(f"PandaScore Dota2 error: {e}")
        return ""
