...
OPENDOTA_KEY     = os.environ["OPENDOTA_KEY"]
...
PLANS = {
    "week":  {"name": "🗓 Неделя",  "price": 199,  "days": 7,   "req_per_day": 5},
    "month": {"name": "📅 Месяц",   "price": 599,  "days": 30,  "req_per_day": 10},
    "year":  {"name": "🏆 Год",     "price": 4999, "days": 365, "req_per_day": 15},
}
...
# ══════════════════════════════════════════════════════════════════════════════
#  OPENDOTA — Dota 2 статистика
# ══════════════════════════════════════════════════════════════════════════════
async def get_dota_stats(team1: str, team2: str) -> str:
    """Получаем Dota 2 статистику через OpenDota API — 2 параллельных запроса"""
    try:
        async with httpx.AsyncClient(timeout=8) as client:
            params_base = {"api_key": OPENDOTA_KEY}

            async def fetch_team(name):
                r = await client.get(
                ...
                for t in teams:
                    ...
                    if t_name == name_lower:
                        logger.info(f"OpenDota exact match '{name}' → '{t.get('name')}' (id={t.get('team_id')})")
                        return t
                # Потом частичное — но минимум 4 символа чтобы не было ложных совпадений
                if len(name_lower) >= 4:
                    for t in teams:
                        t_name = (t.get("name") or "").lower().strip()
                        if name_lower in t_name or t_name in name_lower:
                            logger.info(f"OpenDota partial match '{name}' → '{t.get('name')}' (id={t.get('team_id')})")
                            return t
                logger.warning(f"OpenDota: команда '{name}' не найдена")
                return None

            async def fetch_recent_matches(team_id):
                r = await client.get(
                ...
                if r.status_code != 200:
                    return []
                # Сортируем по match_id убыванию — больший ID = свежее матч
                all_matches = r.json()
                all_matches.sort(key=lambda m: m.get("match_id", 0), reverse=True)
                matches = all_matches[:5]
                results = []
                for m in matches:
                    opponent = m.get("opposing_team_name", "?")
                    won = "✅" if m.get("radiant_win") == m.get("radiant") else "❌"
                    results.append(f"{opponent} {won}")
                return results

            # Параллельный поиск обеих команд
            t1, t2 = await asyncio.gather(fetch_team(team1), fetch_team(team2))

            if not t1 or not t2:
                logger.warning(f"OpenDota: не нашли одну из команд — t1={t1}, t2={t2}")
                return ""

            # Если нашли одну и ту же команду — данные некорректны
            if t1["team_id"] == t2["team_id"]:
                logger.warning(f"OpenDota: обе команды разрезолвились в один ID {t1['team_id']} — пропускаем")
                return ""

            # Параллельно тянем матчи
            last1, last2 = await asyncio.gather(
                fetch_recent_matches(t1["team_id"]),
                fetch_recent_matches(t2["team_id"])
            )
            ...
            stats = "📊 Dota 2 статистика (OpenDota):\n"
            stats += (f"{team1}: побед {t1.get('wins', 0)}, "
                      f"поражений {t1.get('losses', 0)}, винрейт {winrate(t1)}\n")
            stats += (f"{team2}: побед {t2.get('wins', 0)}, "
                      f"поражений {t2.get('losses', 0)}, винрейт {winrate(t2)}\n")
            if last1:
        ...
        logger.warning(f"OpenDota error: {e}")
        return ""
