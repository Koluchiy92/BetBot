...
async def get_dota_stats(team1: str, team2: str) -> str:
    """Получаем Dota 2 статистику через Stratz GraphQL — 2 параллельных запроса"""
    try:
        async with httpx.AsyncClient(timeout=8) as client:
            headers = {
            ...
            async def fetch_team(name):
                query = """{ teams(request: {take: 1, teamName: "%s"}) {
                    id name winCount lossCount lastMatchDateTime
                } }""" % name
                r = await client.post(
            ...
            # Параллельно — 2 запроса вместо последовательных
