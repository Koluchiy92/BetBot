...
async def get_odds(team1: str, team2: str, sport: str) -> str:
    ...
    h2h (исход), totals (тотал), spreads (фора), btts (обе забьют — только футбол).
    ...
    # Рынки: h2h + totals + spreads всегда доступны через основной /odds эндпоинт.
    # btts (обе забьют) добавляем только для футбола — для CS2 рынка не существует.
    markets = "h2h,totals,spreads,btts" if sport == "football" else "h2h"
...
2. В "Лучшие находки" включай ТОЛЬКО исходы, где твоя оценка вероятности ≥ 60%.
   Если ни один исход не дотягивает до 60% — не занижай порог и не притягивай
   слабые варианты, просто не выводи их (см. формат ниже).
3. Среди исходов с вероятностью ≥ 60% коэффициент должен быть ≥ 1.60. Если самый
   вероятный исход имеет коэффициент ниже 1.60 (классический фаворитский расклад
   вроде П1 за 1.10) — пропускай его и ищи следующий по вероятности исход с
   приемлемым коэффициентом (двойной шанс, фора, тотал и т.д.), а не предлагай
   ставку с мизерным потенциальным выигрышем.
4. Если данных по рынку нет (например, нет тоталов/фор в API) — оценивай по своим
   знаниям и статистике личных встреч, явно помечая источник оценки.
5. НИКОГДА не включай в находки исход с вероятностью ниже 60%, даже если у него
...
6. КРИТИЧЕСКИ ВАЖНО — выводи ТОЛЬКО финальный результат, без хода рассуждений.
   ...
   слишком низкий кэф", "1X — тоже не подходит" и т.п.). Никакого "черновика мыслей"
   в ответе — все сравнения и отсев делай молча про себя, в ответе только итог.
...
💎 *Лучшие находки (до 3, по убыванию вероятности, только К ≥ 1.60 и вероятность ≥ 60%):*
...
Если нет ни одной — одна строка: "Нет надёжных находок с вероятностью ≥60% и К≥1.60 на этот матч."]

✅ *Лучшая:* [номер и название] — [одна строка почему именно эта наиболее надёжна]

Правила:
- Находки — НЕ обязательно разных типов рынка. Бери САМЫЕ ВЕРОЯТНЫЕ исходы с К ≥ 1.60,
  даже если все из одного рынка. Приоритет — реальная высокая вероятность, а не
  разнообразие рынков или ровное число "три".
- Двойной шанс (1X, X2) и тотал часто дают лучший баланс "высокая вероятность +
  приемлемый коэффициент", чем чистый исход П1/П2 — рассматривай их в первую очередь
  для фаворитов с низким кэфом на чистую победу.
- Коэффициент каждой находки СТРОГО ≥ 1.60, вероятность СТРОГО ≥ 60%. Если у самого
  вероятного исхода кэф ниже 1.60 — пропускай его, ищи следующий по вероятности
  исход с кэфом ≥ 1.60 (например двойной шанс вместо чистой победы).
- Рынки для поиска: исход (П1/Х/П2), двойной шанс (1X/X2), тотал (ТБ/ТМ X.5),
  фора (Ф1/Ф2 ±X.5), обе забьют (Да/Нет). Угловые, удары в створ, тайм-исходы НЕ
  анализируй — для них нет надёжных коэффициентов в данных, не придумывай вероятности
  на пустом месте.
...
- Коэффициенты: используй ТОЛЬКО из блока данных, если он есть. Не выдумывай цифры.
...
async def analyze_screenshot(image_bytes: bytes, plan: str | None, odds: str = "") -> str:
    ...
    user_msg = (
        ...
        "найди до трёх самых вероятных исходов с вероятностью ≥ 60% и К ≥ 1.60 "
        "(если фаворит идёт по слишком низкому кэфу — рассмотри двойной шанс/тотал/фору) "
        "и одну лучшую среди них. Максимум 10 строк."
        f"{odds_block}"
    )
