...
async def get_dota_stats(team1: str, team2: str) -> str:
    ...
    try:
        async with httpx.AsyncClient(timeout=8) as client:
            ...
            # Параллельный поиск обеих команд — 2 запроса одновременно
            # (OpenDota /teams возвращает все команды — фильтруем локально)
...
async def analyze_match(user_text: str, sport: str,
    ...
    msg = (
        ...
        f"{data_prefix}\n"
        f"Сначала сделай веб-поиск по командам и турниру чтобы найти актуальный состав, "
        f"форму и последние результаты. Затем дай полный профессиональный разбор по всем пунктам."
    )
    ...
    # Цикл: Claude может сделать несколько поисков перед финальным ответом
    for _ in range(3):  # максимум 3 итерации
        ...
        # Если Claude хочет сделать поиск — выполняем и продолжаем
            ...
            for block in response.content:
                if block.type == "tool_use":
                    tool_results.append({
                        ...
                        "content": ""  # Anthropic выполняет поиск на своей стороне
                    })
    ...
    # Если вышли из цикла — финальный ответ без инструментов
    response = anthropic_client.messages.create(
        model="claude-sonnet-4-6",
        max_tokens=1200,
        system=system,
