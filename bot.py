...
async def get_odds(team1: str, team2: str, sport: str) -> str:
    ...
    Считаем implied probability по каждому исходу, чтобы Claude мог искать
    расхождение между рыночной оценкой и своей собственной (= потенциальный value)."""
            ...
            for events in results:
                for event in events:
                    ...
                    odds_info += (
                        ...
                        "сравни со своей оценкой вероятности по каждому рынку — "
                        "большое расхождение = потенциальный value)\n"
                    )
                    return odds_info
...
# ══════════════════════════════════════════════════════════════════════════════
SYSTEM_PROMPT = """Ты — спортивный аналитик и эксперт по value-betting. Дата: {current_date}.

Твоя задача — не просто дать прогноз, а найти РАСХОЖДЕНИЕ между коэффициентом букмекера
и реальной вероятностью события. Это и есть value (возможная недооценка рынком).

Как искать value:
1. По каждому рынку, который есть в данных (исход 1X2, тотал, фора, обе забьют) —
   сначала сам оцени вероятность исхода на основе формы, статистики, личных встреч.
2. Сравни свою оценку с implied probability букмекера (она указана в данных как ~%).
3. Если твоя оценка вероятности заметно ВЫШЕ, чем у букмекера — это потенциальный value
   (рынок недооценивает исход, коэффициент выгоднее, чем должен быть).
4. Если данных по рынку нет (например, нет тоталов/фор в API) — оценивай по своим знаниям,
   но явно помечай это как "рыночная оценка", а не value-находку.
5. ВАЖНО — что НЕ является value: если твоя оценка вероятности НИЖЕ или равна implied
   probability букмекера — это НЕ value, а наоборот рынок переоценивает исход (антиvalue).
   Такие исходы НИКОГДА не включай в "Лучшие находки", даже если расхождение большое
   по модулю и даже если находок наберётся меньше трёх. Лучше показать 1-2 находки, чем
   добавить антиvalue для ровного счёта.
6. Минимальный коэффициент для находки — 1.60. Исходы с коэффициентом ниже 1.60 не
   включай в "Лучшие находки", даже если по ним есть положительный value — слишком
   низкий потенциальный выигрыш не оправдывает риск ставки.
...
💎 *Лучшие находки (до 3, по убыванию value, только К ≥ 1.60):*
1️⃣ [Рынок и название] | К X.XX | твоя оценка X% vs рынок ~X% | уверенность X%
2️⃣ [Рынок и название] | К X.XX | твоя оценка X% vs рынок ~X% | уверенность X%
3️⃣ [Рынок и название] | К X.XX | твоя оценка X% vs рынок ~X% | уверенность X%
[Если положительных value-находок с К≥1.60 меньше трёх — выведи столько, сколько реально есть, без добавления антиvalue]

✅ *Лучшая:* [номер и название] — [одна строка почему именно эта недооценена]

Правила:
- Находки — НЕ обязательно разных типов рынка. Бери ЛУЧШИЕ по положительному расхождению
  вероятностей (твоя оценка выше рынка), даже если все из одного рынка (например, три
  разных тотала) — если это даёт максимальный value, так и делай. Приоритет — реальное
  положительное расхождение, а не разнообразие или ровное число "три".
- Коэффициент каждой находки СТРОГО ≥ 1.60. Если у исхода с хорошим value коэффициент
  ниже 1.60 — не включай его, ищи следующий по силе value среди исходов с К ≥ 1.60.
- Рынки для поиска: исход (П1/Х/П2), тотал (ТБ/ТМ X.5), фора (Ф1/Ф2 ±X.5), обе забьют (Да/Нет).
  Угловые, удары в створ, тайм-исходы НЕ анализируй — для них нет надёжных коэффициентов в данных,
  не придумывай вероятности на пустом месте.
- Для хоккея/баскетбола (где нет блока коэффициентов с API): анализируй вратарей/лидеров по очкам,
  форму, статистику — давай рыночную оценку, не выдавай за value без реальных коэффициентов.
- Коэффициенты: используй ТОЛЬКО из блока данных, если он есть. Не выдумывай цифры.
- Уверенность каждой находки от 48% до 75%
...
async def analyze_screenshot(image_bytes: bytes, plan: str | None, odds: str = "") -> str:
    ...
    user_msg = (
        ...
        "найди до трёх лучших value-находок с К ≥ 1.60 (только положительное расхождение — "
        "твоя оценка выше рынка; антиvalue не включай, даже для ровного счёта) "
