...
def build_cache_key(analysis_text: str) -> str | None:
    ...
    match_line = _re.search(
        r'(футбол|football|хоккей|hockey|баскетбол|basketball)[^\n]{0,60}?'
        r'([А-ЯA-Z\w][а-яёa-z\w\s\-\.]{2,}?)\s+(?:vs|против|\u2014)\s+([А-ЯA-Z\w][а-яёa-z\w\s\-\.]{2,})',
        analysis_text, _re.IGNORECASE
    )
    date_match = _re.search(r'(\d{2}\.\d{2}\.\d{4})', analysis_text)
    if not match_line:
        return None
    sport = match_line.group(1).lower().strip()
    team1 = " ".join(match_line.group(2).lower().split())
    team2 = " ".join(match_line.group(3).lower().split())
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        cache_key = build_cache_key(analysis) if is_pro else None
        logger.info(f"Cache check: is_pro={is_pro}, plan={plan}, cache_key={cache_key}")
        if is_pro and not cache_key:
