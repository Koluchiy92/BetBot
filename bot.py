...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not plan:
            detected_sport = detect_sport(analysis)
        ...
        # Ключ строим для всех — Pro пишет, все читают
        cache_key = build_cache_key(analysis)
        if not cache_key:
            logger.info(f"Cache key build FAILED. Analysis start: {analysis[:200]}")
        ...
        if cached:
            ...
            # Пишем в кеш только если Pro
            if is_pro and cache_key:
