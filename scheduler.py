...
async def fetch_football_matches() -> list[dict]:
    ...
    try:
        ...
        for e in all_events:
            ...
            # Название турнира — берём из вложенного поля которое мы добавили выше
            tournament = (e.get("_tournament_name") or
                         e.get("tournament", {}).get("name", "") or
                         e.get("league", {}).get("name", "") or "")

            # Команды — Flashscore использует home_team/away_team или homeTeam/awayTeam
            home = (e.get("home_team") or
                    e.get("homeTeam", {}).get("name") or
                    e.get("home", {}).get("name") or "?")
            away = (e.get("away_team") or
                    e.get("awayTeam", {}).get("name") or
                    e.get("away", {}).get("name") or "?")
            # Если home/away это строка напрямую
