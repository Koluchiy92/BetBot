...
async def fetch_todays_matches() -> dict:
    ...
    football, hockey, basketball = await asyncio.gather(
...
async def morning_post():
    ...
    total = len(football) + len(hockey) + len(basketball)
    if total == 0:
        logger.info(f"No top matches today (football={len(football)}, hockey={len(hockey)}, basketball={len(basketball)}) — skipping morning post")
        return

    # Сохраняем для вечернего разбора
    daily_state["morning_matches"] = football + hockey + basketball

    all_times = []
    for m in (football + hockey + basketball):
        try:
...
async def afternoon_post():
    ...
    if not matches:
        ...
        matches = data["football"] + data["hockey"] + data["basketball"]
