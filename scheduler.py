...
async def fetch_football_matches() -> list[dict]:
    """Топовые футбольные матчи: окно 09:00 сегодня → 09:00 завтра МСК"""
    try:
        now_msk = datetime.now(MSK)
        # Граница окна — 09:00 завтра по МСК
        window_end = now_msk.replace(hour=9, minute=0, second=0, microsecond=0) + timedelta(days=1)
        window_end_ts = int(window_end.timestamp())

        # Sofascore принимает только одну дату — запрашиваем сегодня и завтра
        ...
        async with httpx.AsyncClient(timeout=10) as client:
            for date in dates:
                ...
                if r.status_code != 200:
                    logger.warning(f"Sofascore HTTP {r.status_code} for {date}: {r.text[:200]}")
                    continue
                all_events.extend(r.json().get("events", []))

        logger.info(f"Sofascore returned {len(all_events)} events total")
        matches = []
        for e in all_events:
            start_ts = e.get("startTimestamp")
            if not start_ts or start_ts >= window_end_ts:
                continue
...
ВАЖНО: не более 4000 символов, не более 15 строк итого. Язык: русский. Отвечай ТОЛЬКО текстом поста, без вступлений типа «Вот пост» или «Готово»."""
...
async def afternoon_post():
    ...
    # Матчи в state уже только сегодняшние благодаря UTC-фиксу в fetch_cs2_matches
    match = matches[0]
    sport_emoji = {"football": "⚽", "cs2": "🎮", "dota": "🐉"}[match["sport"]]

    # Формируем блок с составами если есть
    roster_block = ""
    if match.get("team1_players") and match.get("team2_players"):
        r1 = ", ".join(match["team1_players"])
        r2 = ", ".join(match["team2_players"])
        roster_block = f"\nСостав {match['team1']}: {r1}\nСостав {match['team2']}: {r2}\n(Составы актуальные — из API. Используй их точно.)"
    elif match.get("team1_players"):
        r1 = ", ".join(match["team1_players"])
        roster_block = f"\nСостав {match['team1']}: {r1}\nСостав {match['team2']}: неизвестен — напиши 'состав уточняется'."
    elif match.get("team2_players"):
        r2 = ", ".join(match["team2_players"])
        roster_block = f"\nСостав {match['team1']}: неизвестен — напиши 'состав уточняется'.\nСостав {match['team2']}: {r2}"
    else:
        roster_block = "\nСоставы команд: данные недоступны — напиши 'состав уточняется' вместо перечисления игроков."
...
Вид спорта: {match['sport']}{roster_block}
