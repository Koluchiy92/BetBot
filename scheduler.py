...
def _is_top_football(tournament: str) -> bool:
    import re
    t = tournament.lower()
    for top in TOP_FOOTBALL_LEAGUES:
        # Проверяем точное совпадение слова/фразы (не substring)
        pattern = r'(?<![a-z])' + re.escape(top.lower()) + r'(?![a-z])'
...
def _has_top20_team(team1: str, team2: str) -> bool:
    ...
    for nat in FIFA_TOP20:
        # Целое слово — чтобы "usa" не совпало с "sousa", "brazil" с "paraibano" и т.д.
        pattern = r'(?<![a-z])' + re.escape(nat) + r'(?![a-z])'
        if re.search(pattern, t1) or re.search(pattern, t2):
            return True
    return False
...
2. Блоки по видам спорта (только те, по которым есть матчи):
   ...
   - Первая: *время МСК* | *Команда1 — Команда2* | турнир
     Названия команд и стран ВСЕГДА на русском языке (USA→США, France→Франция, Morocco→Марокко и т.д.)
     Если это национальные сборные — добавь флаг-эмодзи перед каждой командой (🇺🇸 США, 🇫🇷 Франция, 🇲🇦 Марокко и т.д.)
