...
TOP_FOOTBALL_LEAGUES = [
    ...
    "FIFA World Cup", "World Cup 2026", "FIFA World Cup 2026",
    "UEFA European Championship", "Copa America",
    "FA Cup", "Nations League",
]
...
async def fetch_football_matches() -> list[dict]:
    ...
    try:
        ...
        for e in all_events:
            ...
            top = _is_top_football(tournament)
            nat = _has_top20_team(home, away, tournament)
            if not top and not nat:
                continue
            logger.info(f"Football PASS: {home} vs {away} | {tournament} | top={top} nat={nat}")
