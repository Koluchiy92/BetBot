...
async def fetch_cs2_matches() -> list[dict]:
    ...
    try:
        # Начало и конец сегодняшнего дня по МСК → переводим в UTC для PandaScore
        now_msk = datetime.now(MSK)
        day_start_msk = now_msk.replace(hour=0, minute=0, second=0, microsecond=0)
        day_end_msk   = now_msk.replace(hour=23, minute=59, second=59, microsecond=0)
        day_start_utc = day_start_msk.astimezone(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
        day_end_utc   = day_end_msk.astimezone(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
        async with httpx.AsyncClient(timeout=10) as client:
            r = await client.get(
                "https://api.pandascore.co/csgo/matches/upcoming",
                params={
                    "range[begin_at]": f"{day_start_utc},{day_end_utc}",
                    "per_page": 20,
                    "sort": "begin_at"
                },
