...
# ══════════════════════════════════════════════════════════════════════════════
#  БЕЗОПАСНАЯ ОТПРАВКА — режем текст под лимит Telegram (4096), берём запас 4000
# ══════════════════════════════════════════════════════════════════════════════
def split_text(text: str, limit: int = 4000) -> list[str]:
    """Режем длинный текст на части по лимиту, стараясь рвать по границам абзацев/строк"""
    if len(text) <= limit:
        return [text]
    parts = []
...
async def send_post(chat_id, text: str, parse_mode=ParseMode.MARKDOWN, reply_markup=None):
    """Отправляем пост с безопасной разбивкой по лимиту Telegram + fallback на plain-текст при ошибке разметки"""
    parts = split_text(text, limit=4000)
    last_msg = None
    for i, part in enumerate(parts):
        ...
        try:
            ...
            logger.warning(f"send_post markdown failed on part {i+1}/{len(parts)}: {e}")
...
async def fetch_cs2_matches() -> list[dict]:
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=10) as client:
            # Запрашиваем upcoming И running — Major матчи могут быть в обоих
            r_upcoming, r_running = await asyncio.gather(
                client.get(
                    "https://api.pandascore.co/csgo/matches/upcoming",
                    params={"range[begin_at]": f"{window_start_utc},{window_end_utc}",
                            "per_page": 20, "sort": "begin_at"},
                ...
                if isinstance(d, list):
                    for m in d:
                        ...
                        if mid not in seen_ids:
                            ...
                            raw.append(m)
            data = raw
            logger.info(f"PandaScore CS2 total (upcoming+running): {len(data)} matches")
            matches = []
            for m in data:
...
ВАЖНО: не более 2500 символов. Язык: русский. Отвечай ТОЛЬКО текстом поста, без вступлений типа «Вот пост» или «Готово»."""
    ...
    if len(post_text) > 2500:
        logger.warning(f"morning_post: длина {len(post_text)} символов превышает заявленный лимит 2500")

    try:
        msg = await send_post(CHANNEL_ID, post_text, parse_mode=ParseMode.MARKDOWN)
        logger.info(f"Morning post sent: message_id={msg.message_id if msg else '?'}")
    except Exception as e:
...
Длина: не более 15 строк, не более 2000 символов."""
    ...
    if response.stop_reason == "max_tokens":
        logger.warning("afternoon_post: ответ обрезан по max_tokens — пост может быть неполным")
    post_text = "".join(
        b.text for b in response.content
        if getattr(b, "type", None) == "text" and b.text
    )
    if len(post_text) > 2000:
        logger.warning(f"afternoon_post: длина {len(post_text)} символов превышает заявленный лимит 2000")
    ...
    try:
        msg = await send_post(CHANNEL_ID, post_text, parse_mode=ParseMode.MARKDOWN)
        logger.info(f"Afternoon post sent: message_id={msg.message_id if msg else '?'}")
    except Exception as e:
...
Стиль: честный. Telegram Markdown. Язык: русский. Отвечай ТОЛЬКО текстом поста, без вступлений типа «Вот пост» или «Готово». Длина: не более 20 строк, не более 2000 символов."""
    ...
    response = anthropic_client.messages.create(**kwargs)
    if response.stop_reason == "max_tokens":
        logger.warning("evening_post: ответ обрезан по max_tokens — пост может быть неполным")
    post_text = "".join(
        b.text for b in response.content
        if getattr(b, "type", None) == "text" and b.text
    ).strip()
    if len(post_text) > 2000 and RESULTS_NOT_FOUND_MARKER not in post_text:
        logger.warning(f"evening_post: длина {len(post_text)} символов превышает заявленный лимит 2000")
    ...
    try:
        await send_post(CHANNEL_ID, post_text, parse_mode=ParseMode.MARKDOWN)
...
Длина: не более 15 строк, не более 1500 символов."""
    ...
    response = anthropic_client.messages.create(**kwargs)
    if response.stop_reason == "max_tokens":
        logger.warning("post_custom_news: ответ обрезан по max_tokens — пост может быть неполным")
    post_text = "".join(
        b.text for b in response.content
        if getattr(b, "type", None) == "text" and b.text
    )
    if len(post_text) > 1500:
        logger.warning(f"post_custom_news: длина {len(post_text)} символов превышает заявленный лимит 1500")
...
Длина: не более 10 строк, не более 800 символов. БЕЗ развёрнутого анализа и ставок."""
    ...
    if len(post_text) > 800:
        logger.warning(f"post_quick_results: длина {len(post_text)} символов превышает заявленный лимит 800")
