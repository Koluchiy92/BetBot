...
TOP_FOOTBALL_LEAGUES = [
    ...
    "FIFA World Cup", "World Cup", "WC 2026", "FIFA WC", "World Cup 2026",
    "FIFA World Cup 2026", "Чемпионат мира", "ЧМ 2026",
    "UEFA European Championship", "EURO", "Copa America",
    "FA Cup", "Copa del Rey", "Nations League", "International Friendly",
]
        ...
        async with httpx.AsyncClient(timeout=10) as client:
            for date in dates:
                ...
                if r.status_code != 200:
                    ...
                    continue
                data = r.json()
                # Flashscore возвращает список в data["data"] или data["results"]
                events = data.get("data") or data.get("results") or data.get("events") or []
                if isinstance(events, list):
                    all_events.extend(events)
                logger.info(f"Flashscore returned {len(events)} events for {date}")

        logger.info(f"Flashscore total events: {len(all_events)}")
        matches = []
        for e in all_events:
            # Flashscore: время в startTimestamp или start_time или startTime
            start_ts = (e.get("startTimestamp") or e.get("start_time") or
                        e.get("startTime") or e.get("scheduled"))
            if not start_ts:
                continue
            if isinstance(start_ts, str):
                try:
                    from datetime import datetime as _dt
                    start_ts = int(_dt.fromisoformat(start_ts.replace("Z", "+00:00")).timestamp())
                except Exception:
                    continue
            ...
            # Название турнира
            tournament = (e.get("tournament", {}).get("name", "") or
                         e.get("league", {}).get("name", "") or
                         e.get("competition", {}).get("name", "") or "")

            # Команды
            home = (e.get("homeTeam", {}).get("name") or
                    e.get("home_team", {}).get("name") or
                    e.get("home", {}).get("name") or "?")
            away = (e.get("awayTeam", {}).get("name") or
                    e.get("away_team", {}).get("name") or
                    e.get("away", {}).get("name") or "?")
            ...
            matches.append({
                ...
                "time": dt,
                "id": e.get("id") or e.get("match_id") or e.get("event_id"),
            })
