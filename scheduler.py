    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=10) as client:
            for date in dates:
                ...
                if events:
                    logger.info(f"Flashscore sample event keys: {list(events[0].keys()) if isinstance(events[0], dict) else type(events[0])}")
                    logger.info(f"Flashscore sample event: {str(events[0])[:500]}")
